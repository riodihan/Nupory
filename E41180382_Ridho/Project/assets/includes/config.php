...
{
    global $koneksi;
    $kdkritik = htmlspecialchars($kr["id_kritik"]);
...
{
    global $koneksi;
    $id_transaksi = htmlspecialchars($tr["id_pemesanan"]);
    ...
    $trs = mysqli_query($koneksi, "INSERT INTO pemesanan VALUES ('$id_transaksi', '$id_pembayaran', '$id_user', '$id_bunga', '$jumlah', now(), '$alamat', '$total', '')");
...
function uploadpembayaran($up)
{
    ...
    $qu = mysqli_query($koneksi, "UPDATE pemesanan SET
                    ID_PEMESANAN = '$idtransaksi',
...
{
    global $koneksi;
    $qu = mysqli_query($koneksi, "DELETE FROM pemesanan WHERE ID_PEMESANAN = '$id'");
    return $qu;
}
