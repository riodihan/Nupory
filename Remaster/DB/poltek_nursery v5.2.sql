...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 17 Jan 2020 pada 08.19
...
('B001', 'K001', 'krisan spray', 10000, 90, 'bunga 1.jpeg', '-', '-', 'Bunga Krisan adalah sejenis tumbuhan berbunga yang sering ditanam sebagai tanaman hias pekarangan atau bunga petik. Tumbuhan berbunga ini mulai muncul pada zaman Kapur.'),
('B002', 'K004', 'Anggrek Bulan Lokal', 5000, 45, '', '', 'Disiram 10x', 'Anggrek Adalah Bunga'),
('B003', 'K003', 'Mawar Jingga', 25000, 50, '', 'link video.com', 'Disiram tiap pagi dan sore', 'Cantik'),
('B004', 'K003', 'Mawar', 25000, 100, '', 'link video.com', 'Disiram tiap pagi dan sore', 'Cantik');
...
('K001', 'adit', 1, 'Webnya bagus banget, tapi terlalu cerah, hehe'),
('K002', 'idris', 1, 'Proses cepat, tapi masih sedikit bingung.');
...
('T001', '01', '03', 'adit', '2020-01-13', 'Probolinggo', 60000, ''),
('T002', '01', '03', 'idris', '2020-01-13', 'Probolinggo', 65000, '');

--
-- Trigger `transaksi`
--
DELIMITER $$
CREATE TRIGGER `berkurang_jika_dikemas` AFTER UPDATE ON `transaksi` FOR EACH ROW BEGIN
	UPDATE bunga, detail_transaksi
    SET bunga.STOK = bunga.STOK - detail_transaksi.JUMLAH
    WHERE NEW.ID_STATUS_TRANSAKSI = "03" AND
    detail_transaksi.ID_TRANSAKSI = NEW.ID_TRANSAKSI AND
    detail_transaksi.ID_BUNGA = bunga.ID_BUNGA;
END
$$
DELIMITER ;
